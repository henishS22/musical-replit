...
class Pagination {
	...
	public async GetEdges(queryArgs, projection) {
	...
	public async GetPageInfo(edges, queryArgs) {
		if (edges.length) {
			const { filter = {}, sort = {} } = queryArgs
			const startCursor: string | null = this._getStartCursor(edges)
			const endCursor: string | null = this._getEndCursor(edges)
	...
	private async _getFilterWithCursor(fromCursorId, filter, operator, sort) {
		const filterWithCursor = { $and: [filter] }
	...
	private async _getSearchPipeline(
		fromCursorId,
		filter,
		first: any,
		operator,
	...
	private _reverseSortDirection(sort) {
		const fieldArr = Object.keys(sort)
	...
	private _getOperator(sort, options = {}) {
	...
	private async _getHasPreviousPage(startCursor, filter, sort) {
		const isSearch = this._isSearchQuery(sort)
		let prevPage
