...
import { GuildedNft, GuildedNftDocument, Subscription, User, UserDocument } from '../schemas/schemas';
...
export class GuildedNftService {
  ...
  constructor(
    @InjectModel(GuildedNft.name) private readonly guildedNftModel: Model<GuildedNftDocument>,
  ...
  async getNftsById(nftId: string) {
    const nft = await this.guildedNftModel.findOne({ _id: new ObjectId(nftId) });
    if (!nft) {
  ...
  async userNonce(networkChainId: string, buyer: string) {
    ...
    const ERC1155_ABI = ["function getUserNonce(address _user) view returns (uint256)",];
    ...
    const contract = new ethers.Contract(guildedTokenAddress, ERC1155_ABI, provider);
  ...
  async signature(owner: string, signDto: any) {
    ...
    const messageHash = ethers.utils.solidityKeccak256(
      ...
      [buyer, tokenId, maxPrice, timestamp, userNonce, marketplace, networkChainId]
    );

    const signature = await adminSigner.signMessage(ethers.utils.arrayify(messageHash));
